...
export async function initializeImapForUser(userId: string) {
  ...
  for (const account of accounts) {
    try {
      ...
      logger.info(`[IMAP Manager] Started connection for ${account.email}`);
    } catch (err) {
      logger.error(`[IMAP Manager] Failed to start ${account.email}:`, err);
    }
  }
...
export async function initializeAllUsers() {
  ...
  logger.info(`[IMAP Manager] Initialized connections for ${users.length} users`);
}