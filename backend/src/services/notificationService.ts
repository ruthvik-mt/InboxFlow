...
export const notifySlack = async (email: any): Promise<boolean> => {
  ...
  if (!SLACK_TOKEN) {
    logger.warn('[Slack] SLACK_TOKEN not set. Skipping.');
    return false;
  }
  if (!channel) {
    logger.warn('[Slack] SLACK_CHANNEL not set. Skipping.');
    return false;
  }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if ((res as any).ok) {
          ...
          logger.warn('[Slack] Non-ok response:', res);
        ...
        if (attempt >= maxRetries) {
          logger.error(`[Slack] Failed after ${maxRetries} retries:`, errorCode);
          slackSentCache.delete(key);
          return false;
        }

        const delay = baseDelay * Math.pow(2, attempt) + Math.floor(Math.random() * 1000);
        logger.warn(`[Slack] Error: ${errorCode}. Retrying in ${delay}ms (attempt ${attempt + 1})`);
        await sleep(delay);
      }
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (resp.status >= 200 && resp.status < 300) {
          logger.info(`[Webhook] POST success ${resp.status}`);
          return true;
        }
        ...
        if (resp.status === 429) {
          ...
          if (ra !== null) {
            logger.warn(`[Webhook] 429 Retry-After: ${ra}ms`);
            await sleep(ra);
          } else {
            const delay = Math.min(baseDelay * Math.pow(2, attempt) + Math.floor(Math.random() * 500), maxDelay);
            logger.warn(`[Webhook] 429 backing off ${delay}ms (attempt ${attempt})`);
            await sleep(delay);
          }
        } else if (resp.status >= 500) {
          const delay = Math.min(baseDelay * Math.pow(2, attempt) + Math.floor(Math.random() * 500), maxDelay);
          logger.warn(`[Webhook] Server ${resp.status}. Retry after ${delay}ms`);
          await sleep(delay);
        } else {
          logger.error(`[Webhook] Non-retryable ${resp.status}`);
        ...
        if (attempt >= maxRetries) {
          logger.error(`[Webhook] Network error after ${maxRetries} retries:`, e?.message);
          webhookSentCache.delete(key);
          return false;
        }
